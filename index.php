...
if (strpos(parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH) ?? '', '/api/whoami') !== false) {
    startSession();
    header('Content-Type: application/json');
    ...
    echo json_encode([
        ...
        'cookie_recebido' => $_COOKIE[SESSION_NAME] ?? 'NENHUM',
        'save_path'     => session_save_path() ?: ini_get('session.save_path'),
        'save_writable' => is_writable(session_save_path() ?: sys_get_temp_dir()),
        'files_na_sessao' => @count(@scandir(session_save_path() ?: '/tmp') ?: []),
        'todas_sessions'=> $_SESSION,
    ]);
    exit;
}
